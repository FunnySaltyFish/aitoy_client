...
internal object CachitoTouhuanMiniGattProtocol : BleDeviceProtocol {
    private val advertisedServiceUuid = Uuid.parse("0000ffb0-0000-1000-8000-00805f9b34fb")
    private val controlServiceUuid = Uuid.parse("0000feb3-0000-1000-8000-00805f9b34fb")
    private val writeUuid = Uuid.parse("0000fed5-0000-1000-8000-00805f9b34fb")
    private val notifyUuid = Uuid.parse("0000fed6-0000-1000-8000-00805f9b34fb")

    override val status = BleProtocolStatus(
        id = "cachito_touhuan_mini_gatt",
    ...
    override fun matches(fingerprint: BleGattFingerprint): Boolean {
        val name = fingerprint.name.trim()
        val hasTraceName = name.startsWith("SN-", ignoreCase = true)
        val hasTraceAdvertisement =
            fingerprint.serviceUuids.contains(advertisedServiceUuid) ||
                    fingerprint.manufacturerData.contains("0x504", ignoreCase = true)
        val hasControlRoute =
            fingerprint.serviceUuids.contains(controlServiceUuid) &&
                    fingerprint.characteristicUuids.contains(writeUuid)
        return hasTraceName && hasTraceAdvertisement && hasControlRoute
    }
    ...
    override fun commandsFor(action: ToyControlAction): List<BleProtocolOperation> =
        when (action) {
            is ToyControlAction.Intensity ->
                listOf(write(commandOrStop(action.value)))
            is ToyControlAction.Combined ->
                listOf(write(commandOrStop(action.intensity)))
            is ToyControlAction.DualMotor ->
                listOf(write(commandOrStop(action.strongestIntensity(status.intensityMax))))
            is ToyControlAction.Pattern ->
                listOf(write(commandOrStop(action.mode.coerceIn(0, status.intensityMax))))
            ToyControlAction.Stop ->
                listOf(write(stopCommand()))
        }

    private fun commandOrStop(intensity: Int): ByteArray =
        if (intensity <= 0) stopCommand() else command(intensity)

    private fun command(intensity: Int): ByteArray {
        val level = (intensity.coerceIn(0, status.intensityMax) * 0.5 + 50).toInt()
        return frame(
            0x82,
            0x00,
            0x00,
    ...
    private fun frame(vararg body: Int): ByteArray {
        val commandId = Random.nextInt(0x64, 0x100)
        val bytes = bytes(0x71, 0x00, 0x0A, commandId, *body)
        return bytes + ((bytes.sumOf { it.toInt() and 0xff } and 0xff).toByte())
    }
